...
const LeadConversion = defineAsyncComponent(() => import('@/views/pipeline/Lead/LeadConversion.vue'))
...
const sortBy = ref()
const orderBy = ref()
const isEditPipeline = ref(false)
...
const editOpportunity = (id: number) => {
  associationPipeline.value = id
  isEditPipeline.value = !isEditPipeline.value
}
...
<template>
  <div>
    <VCard class="text-high-emphasis text-center">
      <LeadConversion
        v-if="isEditPipeline && associationPipeline > 0"
        v-model:isDialogVisible="isEditPipeline"
